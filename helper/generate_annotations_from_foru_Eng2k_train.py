...
# 产生的文件路径和名字
generate_txt = '../sample/FORUEng2k_annotations_train_set.txt'
# 样本存在的位置，分别存在不同的地方，最后取到一个文件
FORUEng2k_dir = ['../dataset/English2k']
...
def generate(dirs_):
    ...
    for dir_ in dirs_:
        ...
        label_dir_full = dir_ + '/word_annotation/'  # 标注文件夹
        img_dir_full = dir_ + '/word_img/'  # 图片文件夹
        all_label_files = [i for i in os.listdir(label_dir_full) if i.endswith('.txt')]
        all_image_files = [i for i in os.listdir(img_dir_full) if i.endswith('.jpg') or i.endswith('.JPG')]
        ...
        for j in range(len(all_label_files)):
            if all_label_files[j].split('.')[0] != all_image_files[j].split('.')[0]:
                print(j)
                print(all_label_files[j].split('.')[0])
                print(all_image_files[j].split('.')[0])
        ...
        for j in range(len(all_label_files)):
            label_file_name = all_label_files[j]
            ...
            with open(label_file, 'r') as f:
                for l in f.readlines():
                    ...
                    if len(each_line) != 5:
                        ...
                        os._exit(0)
                    x1, y1, w, h, class_name = each_line
                    x2 = int(x1) + int(w) - 1
                    y2 = int(y1) + int(h) - 1
                    ...
                    # 写入信息到文件
                    if label_file_name[:-4] != all_image_files[j][:-4]:
                        print('The label file is not consistent to the image file.')
                        os._exit(0)

                    target_file.write('{}\t{}\t{}\t{}\t{}\t{}\n'.format(
                        os.path.join(img_dir_full, all_image_files[j]),
