...
# 产生的文件路径和名字
generate_txt = '../sample/icdar2013_annotations_train_aug_set.txt'
...
def generate(dirs_):
    ...
    for dir_ in dirs_:
        ...
        label_dir_full = dir_ + '/icdar2013_train_augment_gt/'  # 标注文件夹
        img_dir_full = dir_ + '/icdar2013_train_augment/'  # 图片文件夹
        ...
        for j in range(len(all_label_files)):
            if all_label_files[j].split('.')[0][3:] != all_image_files[j].split('.')[0]:
        ...
        for label_file_name in all_label_files:
            ...
            with open(label_file, 'r') as f:
                for l in f.readlines():
                    ...
                    # 写入信息到文件
                    target_file.write('{}\t{}\t{}\t{}\t{}\t{}\n'.format(
                        os.path.join(img_dir_full, label_file_name.replace('txt', 'jpg')[3:]),
