...
def main():
    # 注释文件
    ANNOTATIONS_FILE = '../sample/icdar2013_annotations_test_set.txt'
    ...
    # all_images = sr.sampleResize(all_images, ShortSide)
    ...
    if TextAugRandom:
        for ii in range(AugLoop):
            ...
            for j in tqdm(range(len(all_images))):
                Aug_width = random.choice([True, False])
                Aug_height = random.choice([True, False])
                Aug_resize = False
                Aug_rotate = random.choice([True, False])
                Aug_rotate_height = random.choice([True, False])
                Aug_rotate_width = random.choice([True, False])
                Aug_add_noise = random.choice([True, False])
                Aug_change_light = random.choice([True, False])
                Aug_crop = random.choice([True, False])
                Aug_shift = random.choice([True, False])
                if Aug_width or Aug_height or Aug_resize or Aug_rotate or \
                        Aug_rotate_height or Aug_rotate_width or Aug_add_noise or \
                        Aug_change_light or Aug_crop or Aug_shift:
                    textAugment_random(copy.deepcopy(all_images[j]), Aug_width=Aug_width, Aug_height=Aug_height,
                                       ...
                                       Aug_shift=Aug_shift, iteration=ii)
                else:
                    j -= 1
