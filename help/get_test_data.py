...
def addTest_ResizeInfo(test_img):
    ...
    size_info, pad_info = sr.sampleResize(y=Y, im_size=size, short_side=ShortSide,
                                          long_side=LongSide, resizeh=ResizeH,
                                          resizew=ResizeW, resizeY=True, sizeinfo=True)
...
def generatePickle(anno_file, save_file):
    ...
    test_images = [s for s in all_images if s['imageset'] == 'test']
    print(len(test_images), 'test samples are loaded.')
    test_images = addTest_ResizeInfo(test_images)
    Output = open(save_file, 'wb')
    pickle.dump(test_images, Output, -1)
...
def main():
    ...
    TRAIN_ANNOTATIONS_FILE = '../sample/icdar2013_annotations_train_set.txt'
    TRAIN_SAVE_PICKLE_FILE = '../sample/icdar2013_trainAStest_sample_608_800.pkl'
...
if __name__ == '__main__':
    ...
    GroundTruthPath_train = '../result/' + CNNModel + '/ground-truth-train/'
